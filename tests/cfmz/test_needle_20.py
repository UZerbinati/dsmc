"""
Hard-needle cross-section — high collision frequency ν=20, Kuramoto Vlasov
---------------------------------------------------------------------------
Same as test_7 (Kuramoto mean-field torque F(θ) = −sin(θ − θ_av), ν=20,
high collision regime) but using the hard-needle NTC kernel instead of the
...
Compare with test_7 (same parameters, Maxwell kernel) to observe the effect of
the anisotropic cross-section in the high-collision regime (collision ratio
ν·dt = 1).
"""
import sys
import petsc4py
...
Opt = PETSc.Options()
Print("Running homogeneous CFMZ needle DSMC — hard-needle cross-section (NTC), high ν=20, Kuramoto Vlasov:")
