"""
Plot the isotropic–nematic phase diagram from the results of
``test_needle_27``.

Reads the ``history.pickle`` produced by each temperature in the sweep
and plots the steady-state circular variance (real projective plane)
...
This is a standalone post-processing step: ``test_needle_27`` only runs
the sweep and writes one ``history.pickle`` per temperature, so the
phase diagram can be regenerated (e.g. after restyling) without paying
for the sweep again.
...
# The T schedule lives with the sweep; import it so the two never drift.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from test_needle_27 import T_bath_values
...
if not T_found:
    sys.exit("No history files found — has test_needle_27 been run?")
