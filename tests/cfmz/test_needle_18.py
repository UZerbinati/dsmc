"""
Hard-needle cross-section — Kuramoto meanfield Vlasov force, perturbed IC
--------------------------------------------------------------------------
Same as test_5 (Kuramoto mean-field torque F(θ) = −sin(θ − θ_av), perturbed
IC with amplitude=0.1, wavelength=1) but using the hard-needle NTC kernel
instead of the Maxwell (uniform) kernel.
...
Compare with test_5 (same parameters, Maxwell kernel) to observe the effect of
the anisotropic cross-section on Kuramoto dynamics from a perturbed IC.
"""
import sys
import petsc4py
...
Opt = PETSc.Options()
Print("Running homogeneous CFMZ needle DSMC — hard-needle cross-section (NTC), Kuramoto Vlasov, perturbed IC:")
