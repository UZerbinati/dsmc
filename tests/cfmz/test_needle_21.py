"""
Hard-needle cross-section — Onsager Vlasov potential, ν=4
----------------------------------------------------------
Same as test_8 (Onsager mean-field potential W(θ₁,θ₂) = |sin(θ₁−θ₂)|, ν=4)
...
Compare with test_8 (same parameters, Maxwell kernel) to observe the effect of
the anisotropic cross-section on Onsager ordering dynamics.
"""
import sys
import petsc4py
...
Opt = PETSc.Options()
Print("Running homogeneous CFMZ needle DSMC — hard-needle cross-section (NTC), Onsager Vlasov, ν=4:")
...
def vlasov_force(theta):
    ...
    if comm.Get_rank() == 0:
        ...
        fig.savefig(f"output/test_needle_21_output_cfmz_{collision_type}/vlasov_energy.pdf")
        fig.savefig(f"output/test_needle_21_output_cfmz_{collision_type}/vlasov_energy.png", dpi=400)
        plt.close(fig)
