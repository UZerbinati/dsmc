"""
Hard-needle cross-section — Onsager Vlasov potential, perturbed IC
-------------------------------------------------------------------
Same as test_12 (Onsager mean-field potential W(θ₁,θ₂) = |sin(θ₁−θ₂)|,
perturbed IC with amplitude=0.1, wavelength=1) but using the hard-needle NTC
kernel instead of the Maxwell (uniform) kernel.
...
Compare with test_12 (same parameters, Maxwell kernel) to observe the effect of
the anisotropic cross-section on Onsager ordering from a perturbed IC.
"""
import sys
import petsc4py
...
Opt = PETSc.Options()
Print("Running homogeneous CFMZ needle DSMC — hard-needle cross-section (NTC), Onsager Vlasov, perturbed IC:")
...
info = {"inertia": 1.0,
        ...
        "initial_angle_amplitude": 1e-1,   # amplitude perturbation from uniform
        "initial_angle_shift": -0.3,        # phase shift of perturbation
        "initial_angle_wavelength": 1,      # wavelength of perturbation
       }
...
def vlasov_force(theta):
    ...
    if comm.Get_rank() == 0:
        ...
        fig.savefig(f"output/test_needle_24_output_cfmz_{collision_type}/vlasov_energy.pdf")
        fig.savefig(f"output/test_needle_24_output_cfmz_{collision_type}/vlasov_energy.png", dpi=400)
        plt.close(fig)
