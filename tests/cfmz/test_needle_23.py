"""
Hard-needle cross-section — Onsager Vlasov potential, ν=20
-----------------------------------------------------------
Same as test_10 (Onsager mean-field potential W(θ₁,θ₂) = |sin(θ₁−θ₂)|, high
collision frequency ν=20) but using the hard-needle NTC kernel instead of the
...
Compare with test_10 (same parameters, Maxwell kernel) to observe the effect of
the anisotropic cross-section in the high-collision regime with Onsager forcing.
"""
import sys
import petsc4py
...
Opt = PETSc.Options()
Print("Running homogeneous CFMZ needle DSMC — hard-needle cross-section (NTC), Onsager Vlasov, ν=20:")
...
def vlasov_force(theta):
    ...
    if comm.Get_rank() == 0:
        ...
        fig.savefig(f"output/test_needle_23_output_cfmz_{collision_type}/vlasov_energy.pdf")
        fig.savefig(f"output/test_needle_23_output_cfmz_{collision_type}/vlasov_energy.png", dpi=400)
        plt.close(fig)
