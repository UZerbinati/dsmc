"""
Hard-needle cross-section — pure collision test, no transport
-------------------------------------------------------------
Same as test_0 (pure collision, no transport, uniform IC) but using the
hard-needle NTC kernel instead of the Maxwell (uniform) kernel.
...
Compare with test_0 (same parameters, Maxwell kernel) to observe the effect of
the anisotropic cross-section on equilibration without transport.
"""
import sys
import petsc4py
...
Opt = PETSc.Options()
Print("Running homogeneous CFMZ needle DSMC — hard-needle cross-section (NTC), no transport:")
